    ...
    public function up()
    {
        Schema::create('projects', function (Blueprint $table) {
            ...
            $table->timestamp('deleted_at')
                ->nullable();
            $table->index('reporter_id', 'fk_projects_users_idx');
            $table->index('owner_id', 'fk_projects_users1_idx');
            $table->index('parent_id', 'fk_projects_projects1_idx');
            $table->foreign('reporter_id', 'fk_projects_users')
                ->references('id')
                ->on('users')->onDelete('no action')->onUpdate('no action');
            $table->foreign('owner_id', 'fk_projects_users1')
                ->references('id')
                ->on('users')->onDelete('no action')->onUpdate('no action');
